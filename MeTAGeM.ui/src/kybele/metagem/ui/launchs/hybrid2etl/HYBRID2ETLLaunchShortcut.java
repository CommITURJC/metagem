...
public class HYBRID2ETLLaunchShortcut implements ILaunchShortcut {
	...
	protected ILaunchConfigurationType getConfigurationType() {
		...
		return getLaunchManager().getLaunchConfigurationType(HYBRID2ETLConstants.MODEL_CONFIGTYPE_ID);
	}
	...
	protected ILaunchConfiguration findLaunchConfiguration(String pathIN, ILaunchConfigurationType configType) {
		...
		try {
			...
			for (int i = 0; i < configs.length; i++) {
				...
				if (config.getAttribute(HYBRID2ETLConstants.ATTR_TRANSFORMATION_IN, "").equals(pathIN)) { 
	...
	protected ILaunchConfiguration createConfiguration(String pathIN) {
		...
		try {
			...
			wc.setAttribute(HYBRID2ETLConstants.ATTR_TRANSFORMATION_IN, pathIN);
			wc.setAttribute(HYBRID2ETLConstants.ATTR_TRANSFORMATION_OUT, new Path(pathIN).removeFileExtension().toOSString() + ".etl_model");
			config = wc.doSave();
