...
transformation returns[Object ret2] @init {Object ret=(backtracking==0) ? ei.create("Transformation", true, false) : null;org.antlr.runtime.Token firstToken=input.LT(1);}
	:	('transformation' temp=stringSymbol {ei.set(ret, "name", temp);} 'input' ((temp=metamodel {ei.set(ret, "sourceMetamodels", temp);} (((COMA '	') temp=metamodel {ei.set(ret, "sourceMetamodels", temp);}))*)) 'output' ((temp=metamodel {ei.set(ret, "targetMetamodels", temp);} (((COMA '	') temp=metamodel {ei.set(ret, "targetMetamodels", temp);}))*)) (() {}| () {}) ((temp=decorator {ei.set(ret, "decorators", temp);} ((() temp=decorator {ei.set(ret, "decorators", temp);}))*))? (() {}| () {}) ((temp=rule {ei.set(ret, "rules", temp);} ((() temp=rule {ei.set(ret, "rules", temp);}))*))?)
        {
