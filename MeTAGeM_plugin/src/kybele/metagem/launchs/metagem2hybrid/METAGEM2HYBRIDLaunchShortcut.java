...
public class METAGEM2HYBRIDLaunchShortcut implements ILaunchShortcut {
	...
	protected ILaunchConfigurationType getConfigurationType() {
		...
		return getLaunchManager().getLaunchConfigurationType(METAGEM2HYBRIDConstants.MODEL_CONFIGTYPE_ID);
	}
	...
	protected ILaunchConfiguration findLaunchConfiguration(String pathIN, ILaunchConfigurationType configType) {
		...
		try {
			...
			for (int i = 0; i < configs.length; i++) {
				...
				if (config.getAttribute(METAGEM2HYBRIDConstants.ATTR_TRANSFORMATION_IN, "").equals(pathIN)) { 
	...
	protected ILaunchConfiguration createConfiguration(String pathIN) {
		...
		try {
			...
			wc.setAttribute(METAGEM2HYBRIDConstants.ATTR_TRANSFORMATION_OUT, new Path(pathIN).removeFileExtension().toOSString() + ".sql2003");
			config = wc.doSave();
